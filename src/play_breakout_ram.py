...
@click.option("--max-steps", "max_steps", type=int, default=10000,
...
def play(
    ...
    env = vmas.make_env(
        ...
        grad_enabled=False,
        render_mode="human",
        life_loss_penalty=0.0,  # No penalty for visualization
        tracking_bonus=0.0,     # No bonus for visualization
    ...
    try:
        for episode in range(episodes):
            ...
            while not done and step < max_steps:
                ...
                # Debug first few steps - show actual cyclic-mapped action
                if step < 25:
                    action_val = actions[0, 0, 0].item()
                    # Cyclic discretization (must match Breakout_Ram.py)
                    normalized = (action_val + 1.0) * 0.5
                    scaled = normalized * action_cycles * 3
                    action_idx = int(scaled) % 3
                    discrete = ["NOOP", "RIGHT", "LEFT"][action_idx]
                    print(f"  Step {step}: action={action_val:.3f} -> {discrete}")
