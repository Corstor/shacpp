...
data/$1/$2/$3/mlp/$4/done: venv/bin/python3
	CUBLAS_WORKSPACE_CONFIG=":4096:8" venv/bin/python3 src/experiments.py \
		...
		run --log-grads $(LOGGRAD) --alg-name $1 --env-name $2 --agents $3 --seed $4 --compile $(COMPILE) --episodes $(EPISODES) --train-steps $(TRAIN_STEPS) --train-envs $(TRAIN_ENVS) --eval-steps $(EVAL_STEPS) --eval-envs $(EVAL_ENVS) $(if $(RESTORE_PATH),--restore-path $(RESTORE_PATH))
...
ENVS=transport dispersion sampling discovery breakout breakout_ram pendulum cartpole
AGENTS=1 3 5
...
pendulum-mlp: $(foreach a,$(ALGOS),$(foreach s,$(SEEDS),data/$a/pendulum/1/mlp/$s/done))
cartpole-mlp: $(foreach a,$(ALGOS),$(foreach s,$(SEEDS),data/$a/cartpole/1/mlp/$s/done))
