"""
Play CartPole with a trained policy model.

Usage:
    python play_cartpole.py --policy-path <path_to_best.pkl>

Example:
    python play_cartpole.py --policy-path data/shacwm/cartpole/1/mlp/42/best.pkl
"""
...
@click.option("--max-steps", "max_steps", type=int, default=500,
...
def play(
    ...
    """
    print("=" * 60)
    print("CARTPOLE PLAYER - Trained Model Visualization")
    print("=" * 60)
    ...
    action_size = 2  # 2 continuous action: LEFT, RIGHT
    hidden_size = 64
    ...
    if policy_nn == "mlp":
        ...
        raise ValueError(f"Unknown policy_nn type: {policy_nn}")
    
    # Load model state dict
    checkpoint = torch.load(policy_path, map_location=device, weights_only=False)
    
    # Handle compiled model keys (remove _orig_mod. prefix) and extract policy_state_dict
    ...
    print(f"\n🎮 Creating CartPole environment with rendering...")
    ...
    env = vmas.make_env(
        ...
        n_agents=1,
        action_size=action_size,  # 2 continuous action: LEFT, RIGHT
        seed=42,
        grad_enabled=False,
        render_mode="human",
    )
    ...
    try:
        for episode in range(episodes):
            # Reset environment
            obs_list = env.reset()
            observations = torch.stack(obs_list).transpose(0, 1).to(device)  # (1, 1, 4)
            ...
            while not done and step < max_steps:
                ...
                if step < 10:
                    ...
                    discrete = ["PUSH_LEFT", "PUSH_RIGHT"][action_idx]
