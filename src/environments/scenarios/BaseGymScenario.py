...
class BaseGymScenario(BaseScenario):
    ...
    - _process_agent_action(agent): Change as needed the actions of the agent (discretization or scaling basically, or both)
    - _get_gym_actions(): Get the action to pass to the env.step() method

    Subclasses could override these methods:
    - _apply_reward_modifications(): Change the reward got by the environment
    - _update_obs_cache(obs_array): Normalize observations got from the env.step() method
    - make_world(num_envs, device, **kwargs): Mainly to manage other args from **kwargs
    """
    ...
    def make_world(self, num_envs: int, device: torch.device, **kwargs) -> World:
        ...
        Args:
            ...
            **kwargs: Additional configuration parameters:
                - render_mode: human to debug or None (default: None)
                - async_vectorize: True if the env are Async or False otherwise (default: num_envs > 16 -> true, false otherwise)
                - action_size: Number of actions given by the agents (default: 2)
        ...
        action_size = kwargs.get("action_size", 2)
        agent = Agent(
            name=agent_name,
    ...
    def _get_agent_config(self) -> tuple:
        """
        Return agent configuration for VMAS.
    ...
    def diffreward(self, prevs, acts, nexts):
        ...
        Raises:
            NotImplementedError: Gymnasium environments already provide non-differentiable rewards
        """
